...
→ 并行准备：用户收藏 / rooms.backup_songs 缓存 / search_prompts 搜索
→ 候选随机打乱，排除当前队列和 recent_played（最近 20 首已播）
→ filterPlayableSongs 验证播放地址可解析
→ 按权重（收藏:缓存:提示词 = 5:3:2）随机混入 play_queue（auto_added + backup_source 标记）
